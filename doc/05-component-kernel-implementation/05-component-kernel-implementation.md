...
The representation for this component is a components.queue.Queue of components.map.Map.Pair<String, Double>, chosen because the First-In-First-Out nature of a queue directly mirrors the chronological sequence of a TimedInputQueue, making the kernel methods efficient "thin" wrappers. The representation invariant (Convention) requires that this.rep is never null and that all stored timestamps are non-negative, ensuring the data remains valid for time-based analysis. The abstraction function (Correspondence) defines that the abstract state of the TimedInputQueue is exactly equal to the sequence of pairs held within the internal rep queue, where the front of the representation corresponds to the first event in the timed sequence. This selection simplifies implementation by leveraging existing OSU components while maintaining strict adherence to the mathematical model of a string of pairs.
